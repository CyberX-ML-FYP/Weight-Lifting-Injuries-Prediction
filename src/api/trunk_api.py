"""
Trunk & Spine Analysis — Production REST API.

Exposes the existing, already-validated Trunk & Spine prediction pipeline
(``src.models.trunk_predict.predict_video``) over HTTP so external clients
can submit one lift video and receive a structured prediction report —
without reimplementing any of the pipeline logic.
...
Scope: this module belongs to the Trunk & Spine (Module 1) pipeline only.
It does NOT modify, import from, or duplicate logic from
``module3_arm_analysis``, ``hip_knee_*``, the frontend, or any other
teammate module, and it does NOT retrain or alter the trained model.
"""
...
from src.features.trunk_config import INTERIM_DIR, VIDEO_EXTENSIONS
from src.features.trunk_logger import get_logger
from src.models.trunk_predict import (
...
# pool, so this lock serializes the actual inference call.
...
app = FastAPI(
    title="Trunk & Spine Lift-Quality Prediction API",
    description=(
        "Upload a lift video and receive the full Trunk & Spine analysis: LSTM "
        "prediction, Rule A-D biomechanics scores, anthropometric "
        "normalization, confidence weighting, pose smoothing, and an "
...
class RuleConfidences(BaseModel):
    """Per-rule confidence scores backing ``confidence_weighted_score``."""
...
class PredictResponse(BaseModel):
    ...
    rule_a: float = Field(..., description="Rule A (vertical stability) score, 0-100.")
    rule_b: float = Field(..., description="Rule B (postural consistency) score, 0-100.")
    rule_c: float = Field(..., description="Rule C (bilateral symmetry) score, 0-100.")
    rule_d: float = Field(..., description="Rule D (recovery smoothness) score, 0-100.")
    confidence_weighted_score: float = Field(..., description="Rule A-D score blended by per-rule confidence, 0-100.")
    vertical_stability: float = Field(..., description="Mean absolute spine-angle deviation from vertical, degrees.")
    postural_consistency: float = Field(..., description="Variance of the spine-angle signal across the lift.")
    bilateral_symmetry: float = Field(..., description="Mean normalized shoulder/hip height asymmetry.")
    recovery_smoothness: float = Field(..., description="Peak trunk angular velocity, degrees/second.")
    torso_length_reference_px: float = Field(..., description="Anthropometric reference length used for normalization.")
...
def _report_to_response(report: PredictionReport, json_path: Path, video_path: Path) -> PredictResponse:
    """Map the existing ``PredictionReport`` dataclass onto the API's
    response schema — no analysis logic lives here, this only reshapes
    ...
    return PredictResponse(
        ...
        vertical_stability=report.vertical_stability,
        postural_consistency=report.postural_consistency,
        bilateral_symmetry=report.bilateral_symmetry,
        recovery_smoothness=report.recovery_smoothness,
        torso_length_reference_px=report.torso_length_reference_px,
        rule_confidences=RuleConfidences(
...
def _save_upload(upload: UploadFile, destination: Path) -> None:
    """Stream the uploaded file to disk in chunks, enforcing
    ``MAX_UPLOAD_BYTES`` without reading the whole file into memory first."""
    total = 0
    try:
...
def root() -> dict[str, Any]:
    return {
        "service": "Trunk & Spine Lift-Quality Prediction API",
        "version": app.version,
        "docs": "/docs",
        "health": "/health",
...
def predict(video: UploadFile = File(..., description="Lift video file (.mp4 or .mov).")) -> PredictResponse:
    """Run the full Trunk & Spine prediction pipeline on an uploaded video.

    NOTE: this footage-agnostic endpoint uses UNSEEDED subject tracking (no
    manually-confirmed anchor exists for a freshly-uploaded video). On
    crowded/wide-shot footage with multiple people visible, unseeded
    tracking is less reliable than the anchor-seeded batch pipeline used
    for the training dataset — see ``trunk_anchor_tool.py``. Prefer
    single-subject or tightly-framed footage when using this endpoint.
    """
...
if __name__ == "__main__":
    ...
    uvicorn.run("src.api.trunk_api:app", host="0.0.0.0", port=8001, reload=False)
